...
a) Simulation of a thin section of rock with 103 particles uniformly magnetized (dipolar sources) by two different induced fields. The average directions of the induced fields are Dec=0\textdegree/Inc=0\textdegree and Dec=180\textdegree/Inc=0\textdegree, yielding two stable directions. b) All estimated vector directions of the identified sources ($M=99$). c) The Filtering criteria used to select the magnetic directions of the sources with the best fitting ($M=96$), determined by the coefficient of determination ($R^ 2 \geq 0.85$) and signal-to-noise ratio (SNR $ \geq 5$).
}
...
spacing of 2.35 $\mu$m and a sensor-sample distance of approximatly 5 $\mu$m, totaling
576x10\textsuperscript{3} data points. The QDM is housed in a shielded room, in
order to avoid the influence of the Earth's magnetic field while the data was taken in projected magnetic microscopy (PMM) mode and converted to the vertical component of magnetic field ($B_{z}$) using a spectral approach \citep{Fu2020,
Glenn2017}. We applied a bias field of 0.9 mT during the measurement, which was periodically reversed to result in an effective background field of <1 µT.  After applying the magnetic anomaly detection algorithm, it was
...
the proximity of the sensor to the sample, it is worth mentioning that the
...
The image re-scaling and blob detection through the Laplacian of Gaussian method was performed with the scikit-image library \citep{VanderWalt2014}. 
